...
int main(int argc, char* argv[])
{
   ...
   ("port,p", boost::program_options::value<int>(),
    "listen port of group sig and ring sig RPC")
   ("http_thread_num,n", boost::program_options::value<int>(),
   "thread num of http server used to deal with requests")
   ("log_path,l", boost::program_options::value<std::string>(),
    "path of log configuration")
   ("ssl_cert,s", boost::program_options::value<std::string>(),
   "path of ssl certification")
   ("ssl_key,k", boost::program_options::value<std::string>(),
   "key to access ssl certification")
   ("help,h", "help of group sig and ring sig rpc");
   ...
   std::cout<<"port:"<<port<<" "<<"thread:"<<http_thread_num<<std::endl;
