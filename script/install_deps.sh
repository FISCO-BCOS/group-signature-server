...
uname -v > /dev/null 2>&1 || { echo >&2 "ERROR - group_sig_service requires 'uname' to identify the platform."; exit 1; }
...
Darwin)
    case $(sw_vers -productVersion | awk -F . '{print $1"."$2}') in
        10.9)
            echo "Installing group_sig service dependencies on OS X 10.9 Mavericks."
            ;;
        10.10)
            echo "Installing group_sig service dependencies on OS X 10.10 Yosemite."
            ;;
        10.11)
            echo "Installing group_sig service dependencies on OS X 10.11 El Capitan."
            ;;
        10.12)
            echo "Installing group_sig service dependencies on macOS 10.12 Sierra."
            echo ""
    ...
    brew -v > /dev/null 2>&1 || { echo >&2 "ERROR - group_sig service requires a Homebrew install.  See http://brew.sh."; exit 1; }
...
#------------------------------------------------------------------------------
FreeBSD)
    echo "Installing group_sig service dependencies on FreeBSD."
...
#------------------------------------------------------------------------------
    ...
    if [ -f "/etc/arch-release" ]; then
        ...
        echo "Installing group_sig service dependencies on Arch Linux."
        ...
        Debian*)
            echo "Installing group_sig service dependencies on Debian Linux."
...
#------------------------------------------------------------------------------
        ...
        CentOS*)
            echo "Installing group_sig service dependencies on CentOS."
        ...
        *)
        ...
        # Alpine Linux
        echo "Installing group_sig service dependencies on Alpine Linux."
...
#------------------------------------------------------------------------------
        openSUSE*)
            #openSUSE
            echo "Installing group_sig service dependencies on openSUSE."
